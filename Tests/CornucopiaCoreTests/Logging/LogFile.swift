...
class Logging: XCTestCase {
    ...
    // Android has no global writable `/tmp`; use CornucopiaCore's own portable helper instead.
    private static let loggingTestDir = FileManager.CC_pathInTempDirectory(suffix: "logging-test") + "/"

    func testLazy() throws {
        let path = Self.loggingTestDir + "lazy.txt"
    ...
    func testNotLazy() throws {
        let path = Self.loggingTestDir + "notlazy.txt"
        try? FileManager.default.removeItem(atPath: path)
        let lf = try Cornucopia.Core.LogFile(path: path, header: "=== This is our fancy log file header ===\n", lazy: false)
        lf.shutdown()
    ...
    func testSimple() throws {
        ...
        let lf = try Cornucopia.Core.LogFile(path: Self.loggingTestDir + "simple.txt", header: "=== This is our fancy log file header ===\n", lazy: true)
    ...
    func testConcurrent() throws {
        ...
        let lf = try Cornucopia.Core.LogFile(path: Self.loggingTestDir + "concurrent.txt", header: "=== This is our fancy log file header ===\n", lazy: true)
        for _ in 0...100 {
            Thread {
    ...
    func testWithoutExplicitShutdown() throws {
        ...
        let lf = try Cornucopia.Core.LogFile(path: Self.loggingTestDir + "noshutdown.txt", header: "=== This is our fancy log file header ===\n", lazy: true)
        lf.log("Let me log something…\n")
    }

    func testLogAfterShutdown() throws {

        let lf = try Cornucopia.Core.LogFile(path: Self.loggingTestDir + "aftershutdown.txt", header: "=== This is our fancy log file header ===\n", lazy: true)
        lf.log("Let me log something…\n")
        lf.shutdown()
    ...
    func testLogOverAnExistingFile() throws {
        ...
        let lf1 = try Cornucopia.Core.LogFile(path: Self.loggingTestDir + "double.txt", header: "=== This is our fancy log file header ===\n", lazy: true)
        lf1.log("Let me log something…\n")
        lf1.shutdown()

        let lf2 = try Cornucopia.Core.LogFile(path: Self.loggingTestDir + "double.txt", header: "=== This is our fancy log file header ===\n", lazy: true)
    ...
    func testRollingTimestamp() throws {
        ...
        let path = Self.loggingTestDir + "timestamps.txt"
        try? FileManager.default.removeItem(atPath: path)
    ...
    func testRollingTimestamp2() throws {
        ...
        @Sendable func sleep(forTimeInterval t: TimeInterval) {
            ...
            // Cross-thread RunLoop/Timer scheduling isn't reliable on non-Darwin Foundation ports.
        ...
        let path = Self.loggingTestDir + "timestamps2.txt"
        try? FileManager.default.removeItem(atPath: path)
